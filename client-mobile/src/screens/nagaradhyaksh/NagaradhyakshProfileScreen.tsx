...
const LINKS: { key: keyof ProfileStackParamList; label: string }[] = [
  { key: 'DepartmentPerformance', label: 'Department Performance' },
  { key: 'Analytics', label: 'Complaint Analytics' },
  { key: 'Garbage', label: 'Garbage Monitoring' },
  { key: 'CityReport', label: 'City Report' },
  { key: 'Settings', label: 'Notification settings' },
];

// P13 — role and scope are read-only. Language is set once on the Landing page, not per-role.
// Also hosts links to the screens that have no dedicated tab slot (Department Performance,
// Analytics, Garbage, City Report, Settings) — same pattern as Officer's Performance link and
// Nagarsevak's Ward Report link.
export function NagaradhyakshProfileScreen({ navigation }: Props) {
    ...
    <View style={styles.root}>
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent}>
        <View style={styles.card}>
          ...
          <View>
            <Text style={styles.name}>{user?.name ?? 'Nagaradhyaksh'}</Text>
            <Text style={styles.mobile}>+91 {user?.mobile}</Text>
          </View>
        </View>

        <View style={styles.card}>
          <Row label="Role" value="Nagaradhyaksh" />
          <Row label="Scope" value="Whole city" last />
        </View>

        {LINKS.map(({ key, label }) => (
          <Pressable key={key} style={({ pressed }) => [styles.menuRow, pressed && styles.menuRowPressed]} onPress={() => navigation.navigate(key)}>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 52, height: 52, borderRadius: 26, backgroundColor: colors.redLight, alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 17, fontFamily: fonts.sansExtraBold, color: colors.red },
  ...
  menuRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.white, borderWidth: 1, borderColor: colors.border, borderRadius: 14, padding: 15 },
  menuRowPressed: { opacity: 0.7 },
  menuLabel: { fontSize: 14, fontFamily: fonts.sansBold, color: colors.text },
