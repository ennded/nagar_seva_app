...
type Props = NativeStackScreenProps<RootStackParamList, 'NagaradhyakshOtp'>;

const KEYS = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', 'del'];
const RESEND_COOLDOWN_SECONDS = 30;

// P2 — six digits on the keypad, then Verify lands on the city overview.
export function NagaradhyakshOtpScreen({ route, navigation }: Props) {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (!data) return;
      if (data.verifyOtp.user.role !== 'NAGARADHYAKSH') {
        setFormError('This mobile number is not registered as a nagaradhyaksh.');
        setCode('');
        return;
      }
      login(data.verifyOtp);
      navigation.reset({ index: 0, routes: [{ name: 'NagaradhyakshTabs' }] });
    } catch (err) {
    ...
    <Screen scroll={false}>
      <View style={styles.top}>
        <View style={styles.iconWrap}>
          <ShieldCheck size={25} color={colors.red} />
        </View>
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.redLight, alignItems: 'center', justifyContent: 'center' },
  ...
  boxActive: { borderColor: colors.red },
  ...
  verifyButton: { marginTop: 16, width: '100%', minHeight: 54, borderRadius: 12, backgroundColor: colors.red, alignItems: 'center', justifyContent: 'center' },
