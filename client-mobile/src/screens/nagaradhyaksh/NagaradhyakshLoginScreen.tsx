...
type Props = NativeStackScreenProps<RootStackParamList, 'NagaradhyakshLogin'>;

// P1 — one account per municipality, provisioned by the Admin.
export function NagaradhyakshLoginScreen({ navigation }: Props) {
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      navigation.navigate('NagaradhyakshOtp', { mobile });
    } catch (err) {
  ...
  return (
    <Screen>
      <View style={styles.iconWrap}>
        <BarChart3 size={24} color={colors.red} />
      </View>
      <Text style={styles.title}>Nagaradhyaksh Login</Text>
      ...
      <View style={styles.form}>
        ...
        <Text style={styles.note}>One account per municipality, provisioned by the Municipal Admin.</Text>
      </View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  iconWrap: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.redLight, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 24 },
  ...
  inputBoxActive: { borderColor: colors.red },
  ...
  sendButton: { marginTop: 18, minHeight: 54, borderRadius: 12, backgroundColor: colors.red, alignItems: 'center', justifyContent: 'center' },
