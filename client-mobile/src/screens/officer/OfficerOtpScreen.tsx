...
type Props = NativeStackScreenProps<RootStackParamList, 'OfficerOtp'>;

const KEYS = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', 'del'];
const RESEND_COOLDOWN_SECONDS = 30;

// O2 — six-digit code with a real inline error state (server-verified, not the mockup's demo
// "123456 is always wrong" stub) plus a resend countdown.
export function OfficerOtpScreen({ route, navigation }: Props) {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (!data) return;
      if (data.verifyOtp.user.role !== 'OFFICER') {
        setFormError('This mobile number is not registered as an officer.');
        setCode('');
        return;
      }
      login(data.verifyOtp);
      navigation.reset({ index: 0, routes: [{ name: 'OfficerTabs' }] });
    } catch (err) {
    ...
    <Screen scroll={false}>
      <View style={styles.top}>
        <View style={styles.iconWrap}>
          <ShieldCheck size={25} color={colors.purple} />
        </View>
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.purpleLight, alignItems: 'center', justifyContent: 'center' },
  ...
  verifyButton: { marginTop: 16, width: '100%', minHeight: 54, borderRadius: 12, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center' },
