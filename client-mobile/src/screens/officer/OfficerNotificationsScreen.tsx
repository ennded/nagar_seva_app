...
type Props = BottomTabScreenProps<OfficerTabParamList, 'AlertsTab'>;

// O8 — assignment and status alerts; tapping one with a linked request opens that complaint's
// detail directly (cross-tab navigation into the Complaints tab's own stack).
export function OfficerNotificationsScreen({ navigation }: Props) {
  ...
  function handlePress(n: Notification) {
    ...
    if (n.requestId) {
      navigation.navigate('ComplaintsTab', { screen: 'ComplaintDetail', params: { id: n.requestId } });
    }
  }
          ...
          notifications.map((n) => (
            ...
            >
              <View style={styles.iconWrap}>
                <Bell size={15} color={colors.purple} />
              </View>
              <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  rowUnread: { backgroundColor: colors.purpleLight },
  rowPressed: { opacity: 0.7 },
  iconWrap: { width: 34, height: 34, borderRadius: 10, backgroundColor: colors.purpleLight, alignItems: 'center', justifyContent: 'center' },
