...
// O10 — department and ward scope are read-only (the Admin sets them). Language is set once on
// the Landing page, not per-role.
export function OfficerProfileScreen({ navigation }: Props) {
  const { session, logout } = useAuth();
  const user = session?.user;

  function handleLogout() {
    logout();
    // Three navigators deep (Tab -> ProfileStack -> this screen) so resetting via the local
    // `navigation` prop can't reach the root's 'Landing' route without threading a composite
    // type through every nested navigator — going through the shared root ref instead.
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent}>
        ...
        <Pressable style={({ pressed }) => [styles.menuRow, pressed && styles.menuRowPressed]} onPress={() => navigation.navigate('Performance')}>
          <Text style={styles.menuLabel}>My Performance</Text>
          <Text style={styles.menuChevron}>›</Text>
        </Pressable>

        <Text style={styles.note}>Department and ward scope are set by the Municipal Admin and cannot be changed here.</Text>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 52, height: 52, borderRadius: 26, backgroundColor: colors.purpleLight, alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 17, fontFamily: fonts.sansExtraBold, color: colors.purple },
