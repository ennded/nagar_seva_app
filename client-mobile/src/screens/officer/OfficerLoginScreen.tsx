...
type Props = NativeStackScreenProps<RootStackParamList, 'OfficerLogin'>;

// O1 — same auth shape as every staff role: mobile number, then OTP. No staff self-registration.
export function OfficerLoginScreen({ navigation }: Props) {
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      navigation.navigate('OfficerOtp', { mobile });
    } catch (err) {
  ...
  return (
    <Screen>
      ...
      <View style={styles.form}>
        ...
        <Text style={styles.note}>Staff accounts are created by the Municipal Admin. There is no staff self-registration.</Text>
      </View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  iconWrap: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.purpleLight, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 24 },
  ...
  inputBoxActive: { borderColor: colors.purple },
  ...
  sendButton: { marginTop: 18, minHeight: 54, borderRadius: 12, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center' },
