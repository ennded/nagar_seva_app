...
// N10 — new ward complaints and status changes; tapping one with a linked request opens its
// read-only detail (cross-tab navigation into the Complaints tab's own stack).
export function NagarsevakNotificationsScreen({ navigation }: Props) {
          ...
          notifications.map((n) => (
            ...
            >
              <View style={styles.iconWrap}>
                <Bell size={15} color={colors.amber} />
              </View>
              <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  rowUnread: { backgroundColor: colors.amberLight },
  rowPressed: { opacity: 0.7 },
  iconWrap: { width: 34, height: 34, borderRadius: 10, backgroundColor: colors.amberLight, alignItems: 'center', justifyContent: 'center' },
