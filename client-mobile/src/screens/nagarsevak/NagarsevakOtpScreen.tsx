...
type Props = NativeStackScreenProps<RootStackParamList, 'NagarsevakOtp'>;

const KEYS = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', 'del'];
const RESEND_COOLDOWN_SECONDS = 30;

// N2 — six digits on the keypad; Verify lands on the ward dashboard.
export function NagarsevakOtpScreen({ route, navigation }: Props) {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (!data) return;
      if (data.verifyOtp.user.role !== 'NAGARSEVAK') {
        setFormError('This mobile number is not registered as a nagarsevak.');
        setCode('');
        return;
      }
      login(data.verifyOtp);
      navigation.reset({ index: 0, routes: [{ name: 'NagarsevakTabs' }] });
    } catch (err) {
    ...
    <Screen scroll={false}>
      <View style={styles.top}>
        <View style={styles.iconWrap}>
          <ShieldCheck size={25} color={colors.amber} />
        </View>
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.amberLight, alignItems: 'center', justifyContent: 'center' },
  ...
  verifyButton: { marginTop: 16, width: '100%', minHeight: 54, borderRadius: 12, backgroundColor: colors.amber, alignItems: 'center', justifyContent: 'center' },
