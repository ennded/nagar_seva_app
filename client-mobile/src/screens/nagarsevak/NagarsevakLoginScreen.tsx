...
type Props = NativeStackScreenProps<RootStackParamList, 'NagarsevakLogin'>;

// N1 — the ward comes attached to the account, never chosen at login.
export function NagarsevakLoginScreen({ navigation }: Props) {
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      navigation.navigate('NagarsevakOtp', { mobile });
    } catch (err) {
  ...
  return (
    <Screen>
      <View style={styles.iconWrap}>
        <Users size={24} color={colors.amber} />
      </View>
      ...
      <View style={styles.form}>
        ...
        <Text style={styles.note}>Your ward is attached to this account by the Municipal Admin.</Text>
      </View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  iconWrap: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.amberLight, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 24 },
  ...
  inputBoxActive: { borderColor: colors.amber },
  ...
  sendButton: { marginTop: 18, minHeight: 54, borderRadius: 12, backgroundColor: colors.amber, alignItems: 'center', justifyContent: 'center' },
